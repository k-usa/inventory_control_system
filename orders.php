            ...
            <div class="nabvar-wrapper">
              <div class="navbar-minimize d-flex align-item-center">
                ...
                <span class="align-center">
                  <a href="orders.php" class="text-decoration-none text-muted h4 text-center"><i class="fas fa-cart-arrow-down"></i> Order</a>
                </span>
              </div>
            ...
            <div class="collapse navbar-collapse" id="menu">
              <ul class="navbar-nav ml-auto">
				...
				<div class="container">
					...
					<div class="border border-dark pl-3 mt-5">
						<div class="row ml-2 my-4">
							<form action="" method="post" class="form-inline">
								<label for="purchaseQty" class="form-label">Purchase Qty</label>
								<input type="number" name="purchaseQty" id="purchaseQty" class="form-control mx-3">
								<button type="submit" class="btn btn-primary" name="calculate" ><i class="fas fa-calculator"></i></button>           
							</form>
						</div>
						<div class="row">
							<?php if($item_id==0)
								...
								elseif(isset($_POST["calculate"]) && !empty($_POST["purchaseQty"]))
								{
								foreach($stockInfo as $rowInfo){ ?>
									<div class="col-6">
										<table class="table table-hover table-light">
											<tr>
												<td>Purchase Qty :</td>
												<td class="text-left">
													<?php echo number_format($_POST['purchaseQty'],0);?>
												</td>
											</tr>
											<tr>
												<td>Amount :</td>
												<td class="text-left">
													<?php
														$totalAmount = $rowInfo['purchase_price'] * $_POST['purchaseQty'];
														echo number_format($totalAmount,2);
													?>
													(@<?php echo number_format($rowInfo['purchase_price'],2); ?> * <?php echo number_format($_POST['purchaseQty'],0);?>qty)
												</td>
											</tr>
												<tr>
													<td>Total Qty :</td>
													<td class="text-left">
														<?php
															$totalQty = $rowInfo['sum(sum_qty)'] + $_POST['purchaseQty'];
									...
									<div class="col-4 d-flex align-items-end mb-3">
										<form action="datafile.php" method="post">
											...
											<input type="hidden" name="orderQty" value="<?php echo $_POST["purchaseQty"];?>">
											<input type="hidden" name="inQty" value="0">
											<button type="submit" class="btn btn-danger btn-block" name="order" style="height:70px;"><i class="fas fa-cart-arrow-down"></i> Order</button>
										</form>
									</div>
								</div>										
							<?php }}else{}?>
